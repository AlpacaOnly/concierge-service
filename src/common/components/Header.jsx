...
export const Header = () => {
  ...
  return (
    <div className="h-16 lg:h-20 flex lg:justify-between bg-white shadow-lg shadow-slate-200">
      <button
        className="lg:hidden px-3 sm:ml-6 outline-none"
        type="button"
        id="mobileMenuToggle"
        data-dropdown-toggle="mobileMenuDropdown"
        onClick={() => setMenuDropdownOpened((value) => !value)}
      >
        <OpenIcon className={`${isMenuDropdownOpened ? "hidden" : ""}`} />
      ...
      <div
        ...
        className={`absolute top-16 bottom-0 left-0 right-0 lg:static flex bg-white max-lg:border-t-2 max-lg:py-8 max-lg:w-full ${
          !isMenuDropdownOpened ? "max-lg:hidden" : ""
        }`}
      >
        <ul
          aria-labelledby="mobileMenuToggle"
          className="list-none flex flex-col lg:flex-row max-lg:w-full"
        >
          <TabItem
            text="Личный кабинет"
            link="/panel/settings"
          ...
          <TabItem
            ...
            onClick={onTabItemClick}
          />
          <TabItem text="Помощь" link="/panel/help" icon="help" onClick={onTabItemClick} />
          <TabItem
            text="Справка"
            link="/panel/reference"
            icon="reference"
            onClick={onTabItemClick}
          />
        </ul>
...
function TabItem(props) {
  return (
    <li onClick={props.onClick}>
      <NavLink
        className="h-full flex items-center relative select-none lg:hover:bg-slate-100 transition duration-150 ease-in-out max-lg:py-6 px-3 sm:px-9"
        to={props.link || "#"}
      >
        {({ isActive }) => (
          <>
            <TabIcon name={props.icon} active={isActive} />
            <span
            ...
            <span
              className={`block h-0.5 rounded bg-emerald-700 absolute bottom-0 left-3 right-3 sm:left-9 sm:right-9 ${
...
function SettingsIcon({ active }) {
  return (
    <svg
      ...
      height="30"
      fill="none"
      className={`${active ? "fill-emerald-700" : "fill-black"}`}
    >
...
function ApplicationsIcon({ active }) {
  return (
    <svg
      width="30"
      height="30"
